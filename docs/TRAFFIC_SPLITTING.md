...
The **App name** you pick in [share.streamlit.io](https://share.streamlit.io) becomes the subdomain. Names like ``northstar3`` / ``northstar4`` work with no extra configuration.

## One-time setup: create ``northstar3`` and ``northstar4``

Do the following **twice** (once for instance 3, once for instance 4).
...
3. Set:
   ...
   - **Branch:** ``main``
   - **Main file path:** ``Instance3.py``
   - **App URL (subdomain):** ``northstar3``
...
6. Copy the full secrets block (TOML) and paste it into **northstar3** → **Settings** → **Secrets** → **Save**.
7. Reboot **northstar3** if prompted (or wait for secrets to apply).
8. Open ``https://northstar3.streamlit.app`` and confirm:
   - Sidebar shows **Northstar instance 3**
...
- **Main file path:** ``Instance4.py``
- **App URL:** ``northstar4``
- Secrets copied from primary into **northstar4**
...
1. **Push repo changes** — ``Instance3.py``, ``Instance4.py``, etc. must be on ``main`` so Cloud can deploy them.
2. **Sheet sync** — still only **GitHub Sync → Push** once; all mirrors read the same JSON.
3. **Slide / QR codes** — list all URLs attendees may use:
...
4. Tell attendees: **“Use any one link — they are the same app.”**
5. **Event deep links** work on every mirror, e.g.  
   ``https://northstar3.streamlit.app/?event=Paris%20%286%2F25%2F2026%29``
...
``Instance3.py`` / ``Instance4.py`` already set the instance id. You only need this if you use ``Home.py`` on a mirror URL and want a sidebar label:

```toml
NORTHSTAR_INSTANCE = "3"
```

## Checklist

- [ ] ``northstar3`` and ``northstar4`` deployed and **Running**
- [ ] Secrets copied from primary to each new app
- [ ] Smoke-tested Trial Sign Up + Auto-Grader on each URL
- [ ] All four URLs on the event slide
